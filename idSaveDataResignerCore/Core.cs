...
public class Core(SimpleLogger logger, ProgressReporter progressReporter)
{
    ...
    /// <summary>
    /// Resigns all files in the specified input directory by decrypting them with the original user ID and re-encrypting them
    /// with a new user ID. The resigned files are saved to a newly created output directory, preserving the original folder structure.
    /// </summary>
    /// <param name="inputDir">The path to the directory containing the files to be resigned. All files within this directory and its subdirectories will be processed.</param>
    ...
    /// <param name="userIdOutput">The user ID to use when re-encrypting the files. The resigned files will be encrypted with this user ID.</param>
    /// <param name="cts">A CancellationTokenSource used to cancel the resigning operation. If cancellation is requested, the process will terminate early.</param>
