...
namespace idSaveDataResignerCore.Helpers;
...
public static class IdDeencryption
{
    ...
    public static void DecryptData(Span<byte> outputSpan, ReadOnlySpan<byte> inputSpan, string fileName, string gameCode, string userId)
    {
    ...
    public static void EncryptData(Span<byte> outputSpan, ReadOnlySpan<byte> inputSpan, string fileName, string gameCode, string userId)
    {
        // Check if outputSpan is the correct size
        if (outputSpan.Length != inputSpan.Length + NonceAndTagTotalLength)
            throw new ArgumentException("Invalid outputSpan length.");
